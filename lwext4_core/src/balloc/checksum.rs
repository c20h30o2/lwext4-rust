...
pub fn set_bitmap_csum(sb: &Superblock, bg: &mut BlockGroup, bitmap: &[u8]) {
    ...
    // 设置低 16 位
    bg.inner_mut().block_bitmap_csum_lo = lo_csum.to_le();

    // 如果是 64 位描述符，设置高 16 位
    if sb.group_desc_size() == EXT4_MAX_BLOCK_GROUP_DESCRIPTOR_SIZE {
        bg.inner_mut().block_bitmap_csum_hi = hi_csum.to_le();
    }
}
...
pub fn verify_bitmap_csum(sb: &Superblock, bg: &BlockGroup, bitmap: &[u8]) -> bool {
    ...
    // 验证低 16 位
    if u16::from_le(bg.inner().block_bitmap_csum_lo) != lo_csum {
        return false;
    }

    // 如果是 64 位描述符，验证高 16 位
    if sb.group_desc_size() == EXT4_MAX_BLOCK_GROUP_DESCRIPTOR_SIZE {
        if u16::from_le(bg.inner().block_bitmap_csum_hi) != hi_csum {
            return false;
        }
    }
